...
md"""
## Objective
Let $n\geq 0$ be an integer. Given $n+1$ distinct real points $x_0,\dots, x_n$ and real values $f_0,\dots, f_n$, we seek a polynomial function $p$ of degree at most $n$, such that

$p(x_i) = f_i \text{ for all } i = 0,\dots ,n.$

Such a polynomial function is called a polynomial interpolant.
"""
...
function interpolate(points, values)
	...
	if n != length(values) || n == 0
		error("Input arrays must be of equal length and nonempty.")
	end
	
	if length(unique(points)) < n
		error("Points must be distinct.")
	end
	...
	function (x)
		...
		for k in 1:n
			...
			for l in 1:n
				if l == k continue end
				prod *= (x-points[l])/(points[k]-points[l])
			end
			res += prod
		end
		res
...
begin
	...
	if dist == "Equidistant"
		points = try
			range(-5, 5, length = num)
		catch e
			[0]
		end
	elseif dist == "Chebyshev"
		points = try
			[5cos((2i-1)pi/(2num)) for i in 1:num]
		catch e
			[0]
		end
	elseif dist == "Custom"
		points = try
			eval(Meta.parse('['*custDist*']'))
		catch e
			[0]
		end
	end
	
	if length(points) == 0 points = [0] end

	values = func.(points)
	interp = interpolate(points, values)
	err(x) = abs(func(x)-interp(x))

	plot(xlims = (-5.5,5.5), ylims = (-5.5,5.5), legend = :bottomright)
	plot!(func, -5.5:0.01:5.5, label = "Input Function")
	plot!(interp, -5.5:0.01:5.5, label = "Interpolation")
	if showError
		plot!(err, -5.5:0.01:5.5, label = "Absolute Error")
	end
	scatter!(points, values, colour = :black, label = :none)
end
