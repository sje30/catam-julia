...
# ╔═╡ 0cb10d36-5e22-4441-bd60-f76a26f0a243
...
# ╔═╡ 34f959cd-bd43-4035-8c37-fa440b2ae6de
isfour(x::Any) = false
...
In this case study, I will be using multiple dispatch in order to write new methods for existing functions from Julia. This will allow me to use the natural names for these functions instead of having to mark them as specifically for my new `Permutation` type (e.g. composing with `∘` instead of something like `composepermutation`).
"""
...
# ╔═╡ 97cff599-0cb1-42fd-9ace-da63a2325a13
isfour(x::Number) = (x == 4)
...
md"""
`Permutation`s now have a much more aesthetic and useful display style:
"""
...
md"""
Next, I want to be able to compose permutations, for which I will write new methods for the inbuilt operator `∘`. In order for an arbitrary number of permutations to be composable, I use an inductive definition which mirrors the definition of composition of functions [from the Julia source code](https://github.com/JuliaLang/julia/blob/master/base/operators.jl#L1096), that is:
```julia
∘(f) = f
...
Now I have the setup to be able to list the elements in a symmetric or alternating group of my choosing. Before this however, I will define two more functions which will be useful to me in their construction.
...
md"""
## Generating groups with `Permutation`s
Instead of just symmetric and alternating groups, I would like to be able to generate arbitrary groups, given a list of `Permutation`s as generators. This will require one final use of multiple dispatch, which will be for me to define equality of permutations:
```julia
import Base.:(==)
==(σ::Permutation, τ::Permutation) = (σ.image == τ.image)
```
...
md"""
An interesting point of note here is that nowhere in the function do I use the syntax `==`, and yet the function would not work without it. This demonstrates further the power of multiple dispatch, since the operator `∉` uses `==` to determine whether or not `υ` is an element of `G` (or `newelements`), and I have defined what `==` means for `Permutation`s, so `∉` will automatically work for permutations without me explicitly telling it how to.
